  ...
  methods: {
    async checkSession() {
      try {
        ...
        const checkTokenResponse = await axios.post('https://194.15.113.90/checkToken', { token });
        const user = checkTokenResponse.data.user;
        if (user) {
        const userDetailsResponse = await axios.get(`https://194.15.113.90/userDetails/${user.userId}`);
