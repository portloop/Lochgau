  ...
  methods: {
    async checkSession() {
      try {
        ...
        const checkTokenResponse = await axios.post('http://srv489299.hstgr.cloud/checkToken', { token });
        const user = checkTokenResponse.data.user;
        if (user) {
        const userDetailsResponse = await axios.get(`http://srv489299.hstgr.cloud/userDetails/${user.userId}`);
