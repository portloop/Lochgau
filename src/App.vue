  ...
  methods: {
    async checkSession() {
      try {
        ...
        const checkTokenResponse = await axios.post('http://yourufx.space/checkToken', { token });
        const user = checkTokenResponse.data.user;
        if (user) {
        const userDetailsResponse = await axios.get(`http://yourufx.space/userDetails/${user.userId}`);
